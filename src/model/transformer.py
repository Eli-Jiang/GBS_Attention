...
class TimeSeriesTransformer(nn.Module):
    def __init__(self, c_in, c_out, seq_len, pred_len, d_model=512, n_heads=8, d_ff=2048, n_layers=3, dropout=0.1):
        """
        c_in:     输入特征数（例如单变量预测就是 1）
        c_out:    输出特征数（通常和 c_in 一致）
        ...
        # 最快跑通的做法：把时序长度和特征维度展平，直接用线性层映射到 [pred_len * c_out]
        self.head = nn.Linear(seq_len * d_model, pred_len * c_out)
    ...
    def forward(self, x):
        ...
        # 映射到未来的预测空间
        output = self.head(x)  # 输出: [Batch_Size, pred_len * c_out]
        
        # 重新整理成标准的时序维度: [Batch_Size, pred_len, c_out]
        output = output.reshape(batch_size, self.pred_len, self.c_out)
